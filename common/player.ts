...
export type PlayerMoveAction = {time: number} & (
  | {
...
export type PlayerAttackAction = {time: number} & ({
  attack: AttackType;
  duration: number;
});
...
export class Player {
  ...
  get x(): number {
    ...
    for (let i = 0; i < this.moveActions.length; i++) {
      const action = this.moveActions[i];
      switch (action.moving) {
        ...
        case 'right':
          const distance =
            Config.horizontalMoveSpeed * ((action.duration === 0 ? now - action.time : action.duration) / 1000);
    ...
    for (let i = 0; i < this.moveActions.length; i++) {
      ...
      switch (action.moving) {
        ...
        case 'right':
          ...
          const distance = Config.horizontalMoveSpeed * (time / 1000);
          position = position + distance * (action.moving === 'left' ? -1 : 1);
          break;
      }
    }
  ...
  updateMoving(direction: PlayerDirection, time: number) {
    ...
    this.moveActions.push({moving: direction, time, duration: 0});
    // this.reconcileMoveActions();
  }

  reconcileMoveActions() {
    let position = 0;
    if (this.lastMoveAction.moving !== 'none') {
      return;
    }
    ...
    const newActions: PlayerMoveAction[] = [];
  ...
  get lastAttackAction(): PlayerAttackAction {
    return this.attackActions[this.attackActions.length - 1] || {time: 0, attack: 'none', duration: 0};
  }
  ...
  updateAttack(attack: AttackType, time: number) {
    ...
    this.attackActions.push({attack, time, duration: 0});
  }

  *getAttacks(): Iterable<{attack: AttackType; x: number; y: number}> {
    const now = this.timeNow;
    for (let i = this.attackActions.length - 1; i >= 0; i--) {
      const attack = this.attackActions[i];
      switch (attack.attack) {
        case 'bullet':
          let hit = false;
          for (
            let time = attack.time;
            time < (attack.duration === 0 ? now : attack.time + attack.duration);
            time += 1000 / this.bulletsPerSecond
          ) {
            const bulletDistance = Math.round(((now - time) / 1000) * this.bulletVelocityPerSecond);
