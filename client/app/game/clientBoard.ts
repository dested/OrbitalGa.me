...
export class ClientBoard extends Board {
  ...
  loadBoard(data: SyncMessage) {
    for (const playerData of data.players) {
      const clientPlayer = this.newPlayer(playerData);
      if (playerData.me) {
  ...
  updateBoard(data: SyncMessage) {
    const missingPlayers: ClientPlayer[] = [...this.players];
    for (const playerData of data.players) {
      const clientPlayer = this.players.find(a => a.playerId === playerData.playerId);
      if (clientPlayer) {
        clientPlayer.shipType = playerData.shipType;
        clientPlayer.setMoveActions(playerData.actions);
  ...
  private draw(msDiff: number) {
    ...
    for (const element of this.getStars()) {
      ...
      context.fillRect(element.x, element.y, 16 * element.n, 16 * element.n);
    }

    for (const player of this.players) {
      const ship = AssetManager.assets[player.shipType];
      ...
      for (const element of player.getAttacks()) {
        context.fillStyle = `rgba(255,240,76,.7)`;
        context.fillRect(element.x - 4, element.y - 8, 8, 16);
      }
    }
    context.restore();
  }
  ...
  *getStars(): Iterable<Star> {
    const starX = this.view.starX;
    const starW = starX + this.view.starWidth;
    const starY = this.view.starY;
    const starH = starY + this.view.starHeight;
